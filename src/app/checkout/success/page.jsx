...
function CheckoutSuccessContent() {
    ...
    useEffect(() => {
        ...
        let attempts = 0;
        const maxAttempts = 15; // Thử tối đa 15 lần (30 giây)

        const fetchOrder = async () => {
            try {
                // Gọi API không cần config cookie thủ công, 
                // Axios với { withCredentials: true } sẽ tự gửi cookie của trình duyệt
                ...
                if (data) {
                    setOrder(data);
                    setLoading(false);
                    return; // Dừng interval khi đã có dữ liệu
                }
            } catch (error) {
                console.log("Đang đợi Webhook xác nhận... (lần " + (attempts + 1) + ")");
            }

            attempts++;
            if (attempts >= maxAttempts) {
                setLoading(false); // Ngừng loading nếu quá thời gian
            }
        };

        fetchOrder();
        const interval = setInterval(fetchOrder, 2000); // Polling mỗi 2 giây

        return () => clearInterval(interval);
    }, [order_id]);

    // Giao diện Loading bổ sung để xử lý độ trễ Webhook
    if (loading) {
        return (
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
                <div className="text-center">
                    ...
                    <p className="text-slate-500 text-sm mt-2">Vui lòng đợi trong giây lát, hệ thống đang cập nhật đơn hàng.</p>
                </div>
            </div>
        );
    }
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Link href="/" className="w-full flex items-center justify-center gap-2 bg-white hover:bg-slate-50 text-slate-700 font-medium text-sm py-3 px-4 rounded-xl transition-all border border-slate-200">
