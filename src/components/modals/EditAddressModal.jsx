...
import { updateAddress } from "../../services/addressService"; // <-- Import API cập nhật địa chỉ
import ConfirmDefaultAddressDialog from '../dialog/addressDialog';

const EditAddressModal = ({ isOpen, onClose, onAddressUpdated, addressData, existingAddresses = [] }) => {
  ...
  // Load danh sách tỉnh/thành phố và đồng thời đổ dữ liệu cũ của addressData vào form khi modal mở
  useEffect(() => {
    if (isOpen) {
      const loadData = async () => {
        ...
        if (addressData) {
          ...
          setFormData({
            ...
            // Map từ tiếng Anh ("home", "office") về tiếng Việt để hiển thị đúng nút bấm giao diện
            addressType: addressData.addressType === "home" ? "Nhà" : (addressData.addressType === "office" ? "Văn phòng" : "Nhà"),
            isDefault: addressData.isDefault || false,
          });

          // Tìm và trích xuất danh sách quận/huyện, phường/xã tương ứng với tỉnh/quận cũ để select hoạt động trơn tru
          if (addressData.province) {
            const foundProv = data.find(p => p.name.toLowerCase() === addressData.province.toLowerCase());
            if (foundProv) {
              setDistricts(foundProv.districts || []);
              if (addressData.district) {
                const foundDist = foundProv.districts.find(d => d.name.toLowerCase() === addressData.district.toLowerCase());
                if (foundDist) {
  ...
  // Hàm thực thi gọi API cập nhật địa chỉ thực tế
  const executeUpdateAddress = async () => {
    ...
    try {
      setLoading(true);
      const res = await updateAddress(addressData.id || addressData._id, payload); 
  ...
  // Xử lý khi Submit form gửi dữ liệu cập nhật lên Backend
  const handleSubmit = async (e) => {
    ...
    // Kiểm tra xem có đổi sang làm mặc định không và trước đó đã có địa chỉ mặc định nào khác chưa 
    // (Loại trừ chính địa chỉ đang được sửa ra khỏi danh sách check)
    const otherAddresses = existingAddresses.filter(addr => (addr.id || addr._id) !== (addressData?.id || addressData?._id));
    const hasExistingDefault = otherAddresses?.some(addr => addr.isDefault);

    if (formData.isDefault && hasExistingDefault && !addressData?.isDefault) {
    ...
    <>
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <h2 className="text-xl font-bold mb-6">Cập nhật địa chỉ</h2>
      ...
      <ConfirmDefaultAddressDialog 
        ...
        loading={loading}
        onConfirm={executeUpdateAddress}
      />
    </>
  );
};
